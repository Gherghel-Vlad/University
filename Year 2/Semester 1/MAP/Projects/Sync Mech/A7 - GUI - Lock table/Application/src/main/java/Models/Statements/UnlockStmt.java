...
public class UnlockStmt implements IStmt{
    ...
    public PrgState execute(PrgState state) throws MyException {
        ...
        if(symTbl.isDefined(this.var) && symTbl.get(this.var).getType().equals(new IntType())){
            ...
            if(lockTbl.isDefined(((IntValue) foundIndex ).getVal())){
                if(lockTbl.getLockTable().get(((IntValue) foundIndex ).getVal()) == state.getIdPrgState()){
                    lockTbl.update(((IntValue) foundIndex ).getVal(), -1);
                }
                else{
                    return null;
                }
                ...
                throw new MyException("UnlockStmt: the integer doesnt show any valid lock table entry");

            }
        }
        else{
            throw new MyException("UnlockStmt: variable doesnt exist or isnt of int type");
        }
    ...
    public MyIDictionary<String, IType> typecheck(MyIDictionary<String, IType> typeEnv) throws MyException {
        ...
        if(!typeEnv.lookup(this.var).equals(new IntType())){
            throw new MyException("UnlockStmt: failed typecheck");
