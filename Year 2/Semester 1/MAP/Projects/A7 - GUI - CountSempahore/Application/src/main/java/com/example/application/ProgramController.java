...
public class ProgramController {
    ...
    public void initData(IStmt s, String fileName){
        ...
        try {
            this.prg = new PrgState(new MyStack<IStmt>(), new MyDictionary<String, IValue>(), new MyList<IValue>(), new MyDictionary<StringValue, BufferedReader>(), new MyHeap(), this.stmt, new MySemaphore());
    ...
    @FXML
    private TableView<SemaphoreTableModel> semaphoreTableId;

    @FXML
    private TableColumn<SemaphoreTableModel, String> semaphoreTableIdListColumnId;

    @FXML
    private TableColumn<SemaphoreTableModel, Integer> semaphoreTableIndexColumnId;

    @FXML
    private TableColumn<SemaphoreTableModel, Integer> semaphoreTableValueColumnId;
    ...
    public void initialize(){
        ...
        this.semaphoreTableIndexColumnId.setCellValueFactory(new PropertyValueFactory<>("Index"));
        this.semaphoreTableValueColumnId.setCellValueFactory(new PropertyValueFactory<>("Value"));
        this.semaphoreTableIdListColumnId.setCellValueFactory(new PropertyValueFactory<>("List"));
    ...
    private void clearEverything(){
        ...
        for ( int i = 0; i<this.semaphoreTableId.getItems().size(); i++) {
            this.semaphoreTableId.getItems().clear();
        }
    ...
    private void populateEverything(){
        ...
        try {
            ...
            for (Integer s : this.prg.getSemaphore().getSemaphore().getContent().keySet()) {
                //this.semaphoreTableId.getItems().add(new SemaphoreTableModel(s, this.prg.getSemaphore().getSemaphore().get(s).getKey(), this.prg.getSemaphore().getSemaphore().get(s).getValue().toString()));
                this.semaphoreTableId.getItems().add(new SemaphoreTableModel(s, this.prg.getSemaphore().getSemaphore().get(s).getKey(), this.prg.getSemaphore().getSemaphore().get(s).getValue().toString()));
            }
