...
public class ReadFileStmt implements IStmt{
    ...
    public PrgState execute(PrgState state) throws MyException {
        ...
        if(symTbl.isDefined(this.var_name)){
            ...
            if(val.getType().equals(new IntType())){
                IValue expVal = exp.eval(symTbl, state.getHeap());
    ...
    public MyIDictionary<String, IType> typecheck(MyIDictionary<String, IType> typeEnv) throws MyException {
        ...
        if (typexp.equals(new StringType())) {
            if(typeEnv.isDefined(this.var_name)){
                return typeEnv;
            }
            else{
                throw new MyException("ReadFile: Variable " + this.var_name + " does not exist.");
            }
        }
        else{
            throw new MyException("ReadFile: value given isn't a string type");
        }
    }
