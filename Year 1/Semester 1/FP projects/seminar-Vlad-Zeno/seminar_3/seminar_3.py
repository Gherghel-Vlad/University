...
# Test functions have no params and do not return anything
def test_delete_student():
    ...
    # Delete first and last student in the list
...
def delete_students_menu_ui(student_list):
...
def start_menu_ui():
    ...
    command_dict = {'3': show_all_students_ui, '1': add_student_ui, "2": delete_students_menu_ui}
...
def add_student_command_ui(student_list, command_params):
    ...
    if find_by_id(student_list, params[0].strip()) == -1:
        student_list.append(create_student(params[0].strip(), params[1].strip(), int(params[2].strip())))
    else:
...
def split_command(command):
    """
    Separate user command intro command word and parameters
...
def test_split_command():
    ...
    for cmd in ["add 1, Pop Ionescu, 5", "  AdD 1, Pop Ionescu, 5", "aDd     1, Pop Ionescu, 5", "add 1, Pop Ionescu, 5      "]:
...
def start_command_ui():
    ...
    # 4. call a separate function for each valid command
    student_list = []
    test_init(student_list)
