...
class Graph:
    ...
    def is_okay(self, path, pos, v):
        ...
        # check that the vertex doesnt already exists in the path
        for ver in path:
            if v == ver:
                return False
    ...
    def hamil_recursive(self, path, pos):
        ...
        # going trough all the vertices that have an edge with the previous vertex
        # seeing which can be put on the current pos and going forward with it
    ...
    def check_hamiltonian(self):
        ...
        if self.hamil_recursive(path, 1) == False:
