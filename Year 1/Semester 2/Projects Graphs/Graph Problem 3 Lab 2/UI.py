...
class UI:
    ...
    def print_graph_menu(self):
        ...
        print("14 Show connected components of the graph")
        print("15 Save")
        print("0 Exit\n")
        ...
        while not done:
            ...
            try:
                if command == "1":
                    ...
                    self.show_connected_components_ui();
                elif command == "15":
                    self.modified_file()
                    self.save_graph_to_file()
                elif command == "0":
    ...
    def show_connected_components_ui(self):
        list_of_graphs = self.dfs_connected()

        print(f"\nNumber of connected components: {len(list_of_graphs)}\n")
        print("Connected components: \n")
        for g in list_of_graphs:
            print("Vertices: " + str(list(g.parse_vertices())))
            print("Edges: ", end='')
            already_printed_edges = []
            for ver in g.parse_vertices():
                for v in g.parse_inbound_edges(ver):
                    if (ver, v) not in already_printed_edges:
                        already_printed_edges.append((v, ver))
                        print("(" + str(ver) + ", " + str(v) + ") ", end='')
            print("\n")
    ...
    def dfs_helper(self, x, visited, vertex_list):
        """
        Goes trough the connected part of a graph that contains the vertex v
        :param x: The vertex from which to start
        :param visited: The list of visited elements
        :param vertex_list: The vertex list that contains all the vertices that are connected to v
        :return: -
        """
        # mark the vertex as visited
        visited.append(x)

        # add it to the list so i can create the graph with it after
        if x not in vertex_list:
            vertex_list.append(x)

        # going trough it s neighbors, in a depth style traversal (how i find a vertex, i immediately go to it and do
        # the same
        for v in self._graph.parse_inbound_edges(x):
            if v not in visited:
                self.dfs_helper(v, visited, vertex_list)

    def dfs_connected(self):
        """
        Creates a list of graphs, each being a connected component of the original Graph
        :return: a list of graphs, each being a connected component of the original Graph
        """

        # creating the list of visited vertices
        visited = []
        gr = Graph(0)
        list_of_graphs = [] # list of Graphs representing the connected components found

        # going trough all the vertices
        for v in list(self._graph.parse_vertices()):
            # using the DFS helper function for the vertices that arent visited yet
            if v not in visited:
                vertex_list = []  # initializing it as an empty list
                self.dfs_helper(v, visited, vertex_list)

                # creating the graph with the found vertices and it s corresponding edges
                gr = Graph(0)
                for ver in vertex_list:
                    gr.add_vertex(ver)

                for ver in gr.parse_vertices():
                    for v in gr.parse_vertices():
                        if self._graph.edge_exists(ver, v) and not gr.edge_exists(ver, v):
                            gr.add_edge(ver, v)

                list_of_graphs.append(gr)
