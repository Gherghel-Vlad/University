...
class UI:
    ...
    def print_graph_menu(self):
        ...
        print("14 Shortest path from x to y (dijkstra)")
        print("15 Save")
        print("0 Exit\n")
    ...
    def dijkstra_ui(self):
        try:
            s = int(input("X (source): ").strip())
            t = int(input("Y (target): ").strip())
        except ValueError:
            raise UIException("Bad input!")

        prev = {}
        cost, path = self._graph.dijkstra_alg(s, t)

        print(f"Path from {s} to {t}")
